...
class ItemsController {
  async createItem(req: Request, res: Response) {
    ...
    const item = await Item.create({ ...data, createdBy: req.userId })
  ...
  async getItems(req: Request, res: Response) {
    ...
    query['$or'] = [{ createdBy: req.userId }, { isPublic: true }]
  ...
  async getItemById(req: Request, res: Response) {
    ...
    if (item.createdBy !== req.userId && !item.isPublic) throw new AppError('Access denied', 403)
  ...
  async updateItem(req: Request, res: Response) {
    ...
    const item = await Item.findById(id)
    if (!item) throw new AppError('Item not found', 404)
    if (item.createdBy !== req.userId) throw new AppError('Access denied', 403)
  ...
  async getTags(req: Request, res: Response) {
    if (!req.userId) throw new AppError('Unauthorized', 401)
    const tags = await Item.distinct('tags', { $or: [{ createdBy: req.userId }, { isPublic: true }] })
